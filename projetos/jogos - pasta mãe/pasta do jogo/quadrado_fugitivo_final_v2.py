...
def checar_missoes():
    global popup_texto, popup_timer, meta_atual, estado
    for missao in missoes:
    ...
    if missoes and all(missao["concluida"] for missao in missoes):
...
def desenhar_vitoria():
    ...
    texto_vitoria = fonte_gameover.render("VOCE VENCEU", True, VERDE_VITORIA)
    texto_rect = texto_vitoria.get_rect(center=(LARGURA // 2, 220))
    tela.blit(texto_vitoria, texto_rect)

    subtexto = fonte_pop_titulo.render("Você completou todas as missões!", True, (220, 220, 220))
    subtexto_rect = subtexto.get_rect(center=(LARGURA // 2, 300))
    tela.blit(subtexto, subtexto_rect)
